...
public class JaxbConvertorTest {
	...
	public void unCompressTest() {
		try {
			...
			for(String id : ids) {
				...
				java.io.FileInputStream fis = new java.io.FileInputStream("d:\\tmp\\das\\KFRM\\555508.mer");
				byte[] b = new byte[4];
	...
	@Ignore
	@Test
	public void compressMerWithTxtTest() {
		try {
			
			String ctId = "555508";
			File f = new File("D:/tmp/das/KFRM");
			
			File merFile = new File(f.getAbsolutePath()+File.separator+ctId+".mer");
			if(merFile.exists()) merFile.delete();
			
			BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(new File(f.getAbsolutePath()+File.separator+ctId+".mer")));
			if(f.exists()) {
				String[] fileNames = f.list(new UserFileNameFilter());
				...
				FileWriter fw = null;
				String txtName = f.getAbsolutePath()+File.separator+ctId+".txt";
				File txtFile = new File(f.getAbsolutePath()+File.separator+ctId+".txt");
				if(txtFile.exists()) txtFile.delete();
				
				StringBuffer frames = new StringBuffer();
				for(Integer frame : sorts) {
					// frame number
					System.out.println(frame);
					
					File img = new File(f.getAbsolutePath()+File.separator+frame+".jpg");
				...
				fw = new FileWriter(txtName, true);
				fw.write(frames.toString());
	...
	@Ignore
	@Test
	public void compressMerTest() {
		try {
			File f = new File("D:/tmp/das/KFRM");
			BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(new File("D:\\tmp\\555508.mer")));
			if(f.exists()) {
	...
	public void testDownCart() {
		try {
			...
			downCart.setDownSubj("test");// ?낅젰???ㅼ슫濡쒕뱶 ?붿껌?쒕ぉ(?ъ슜???낅젰)
			//downCart.setNodecaption("媛쒕컻 ?뚯뒪??); // ?대뜑紐?
			downCart.setNodeid(3467);
			downCart.setCellname("PM-CU"); // ?대뜑ID
	...
	public void testEscape() {
		try {
			...
			XmlConvertorService<Das> convertorService = new XmlConvertorServiceImpl<Das>();
			convertorService.unMarshaller("<?xml version=\"1.0\" encoding=\"utf-8\"?><das><monitoring><job_nm>download</job_nm><title>?섎룞?뚯뒪???몄젏3Aa&amp;Bb</title><status>C</status><progress>100</progress><start_time>20130128195014</start_time><app_cont></app_cont><end_time>20130128195207</end_time></monitoring></das>");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
		...
		try {
			...
			if ("archive".equals(workflow)) {
				...
				if(StringUtils.isBlank(identifier)) {
					throw new CMSRuntimeException("?ㅼ슫濡쒕뱶???꾩슂???곸긽ID媛 ?놁뒿?덈떎.");
				}
				...
				Das das3 = new Das();
				downCart = new DownCart();
				downCart.setDownSubj("S52");// ?붿껌???ъ슜?먮챸
				downCart.setCellname(parameters.getProperty("target_storage_id", "")); // ?대뜑ID
				downCart.setDownGubun("007"); // ifcms 援щ텇?? PDS(001), NDS(002), ?곗젙?(003), TAPE_OUT(004)
				downCart.setCartNo(das.getCartContent().getCartNo());
				downCart.setUserId(das.getCartContent().getRegrid());
				downCart.setPhysicaltree(parameters.getProperty("target_folder_id", "")); // 
				downCart.setUrl(callback); // ?낅젰 url
