 ...
 *
 */
public class PreviewDOXML extends DOXml {
	/**
	 * xml해더
	 */ 
	private String XML_NODE_HEAD = "preview";
	/**
	 * 마스터 ID
	 */
	 ...
	 * 제목
	 */
	private String XML_NODE_PREVIEW_SUBJ="preview_subj";
	/**
	 * 내용
	 */
	private String XML_NODE_PREVIEW_CONT="preview_cont";
	/**
	 * 등록일
	 */
	...
	public Object setData(Element pElement) {
		...
		for(int i = 0; i < _length; i++) {
			...
			else if(_nodeName.equals(XML_NODE_PREVIEW_SUBJ)) {
				previewDO.setPreview_subj(_nodeValue);
			}
			else if(_nodeName.equals(XML_NODE_PREVIEW_CONT)) {
				if(_nodeValue != null && _nodeValue.length() > 0) {
					previewDO.setPreivew_cont(_nodeValue);
				}
			}
			else if(_nodeName.equals(XML_NODE_REG_DT)) {
				if(_nodeValue != null && _nodeValue.length() > 0) {
					previewDO.setReg_dt(_nodeValue);
				}
			}
		}
	...
	public String getSubXML() {
		...
		_xml.append("<" + XML_NODE_PREVIEW_SUBJ + ">" + CommonUtl.transXmlText(previewDO.getPreview_subj()) + "</"  + XML_NODE_PREVIEW_SUBJ + "> \n");
		_xml.append("<" + XML_NODE_PREVIEW_CONT + ">" + CommonUtl.transXmlText(previewDO.getPreivew_cont()) + "</"  + XML_NODE_PREVIEW_CONT + "> \n");
		_xml.append("<" + XML_NODE_PATH + ">" + previewDO.getPath() + "</"  + XML_NODE_PATH + "> \n");
		_xml.append("<" + XML_NODE_REG_DT + ">" + previewDO.getReg_dt() + "</"  + XML_NODE_REG_DT + "> \n");
