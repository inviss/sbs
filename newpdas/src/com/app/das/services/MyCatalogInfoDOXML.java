...
/**
 *  내목록 정보 관련 XML파서(멀티저장)
 * @author asura207
 *
 */
public class MyCatalogInfoDOXML extends DOXml {
	MyCatalogDO infoDO;
	/**
	 * xml 해더
	 */
	...
	public Object setData(Element pElement) {
	    ...
	    for(int i = 0; i < _length; i++) {
			...
			if(_nodeName.equalsIgnoreCase(XML_NODE_SERIALNO)) {
				infoDO.setSerialNo(Long.parseLong(_nodeValue));
			}
				...
				infoDO.setCnId(Long.parseLong((_nodeValue)));
			}
				...
				String pgmnm=_nodeValue.replace("'", "`");
				infoDO.setPgmNm(pgmnm);
			}
