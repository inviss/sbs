 ...
 *
 */
public class ErpAppointDOXML extends DOXml {
	/**
	 * xml 헤더
	 */
	private String XML_NODE_HEAD = "erpappoint";
	/** 
	 * 회사코드 
	 */   
	 ...
	 * 발령일자 
	 */  
	private String XML_NODE_ODER_DD="oder_dd";
	/** 
	 * 발령순번 
	 */ 
	private String XML_NODE_ODER_SEQ="oder_seq";
	/** 
	 * 발령코드 
	 */ 
	private String XML_NODE_ODER_CD = "oder_cd";
	/** 
	 * 사용자명 
	 */ 
	 ...
	 * 직책코드 
	 */ 
	private String XML_NODE_JOB_CD="job_cd";
	/** 
	 * 팀장여부 
	 */  
	 ...
	 * 발령 그룹코드
	 */
	private String XML_NODE_OCPN_GR_CD="ocpn_gr_cd";	
	/** 
	 * 발령순번 
	 */   
	...
	public Object setData(Element pElement) {
		...
		for(int i = 0; i < _length; i++) {
			...
			if(_nodeName.equals(XML_NODE_CO_CD)) {
				erpAppointDO.setCo_cd(_nodeValue);
				...
				if(_nodeValue != null && _nodeValue.length() > 0) {
					erpAppointDO.setOder_cd(_nodeValue);
				}
			}
			else if(_nodeName.equals(XML_NODE_USER_NM)) {
				if(_nodeValue != null && _nodeValue.length() > 0) {
					erpAppointDO.setUser_nm(_nodeValue);
				}
			}
				...
				erpAppointDO.setDept_cd(_nodeValue);
			}
				...
				erpAppointDO.setTeam_yn(_nodeValue);
				...
				erpAppointDO.setSeg_cd(_nodeValue);
			}
	...
	public String getSubXML() {
		...
		_xml.append("<" + XML_NODE_ORDER_NM + ">" + CommonUtl.transXmlText(erpAppointDO.getOder_nm()) + "</"  + XML_NODE_ORDER_NM + ">");
		_xml.append("<" + XML_NODE_ORDER_FLAG + ">" + erpAppointDO.getOder_flag() + "</"  + XML_NODE_ORDER_FLAG + ">");
		_xml.append("</" + XML_NODE_HEAD + ">");
